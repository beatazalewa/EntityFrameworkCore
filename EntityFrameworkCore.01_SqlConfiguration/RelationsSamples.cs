...
namespace EntityFrameworkCore._01_SqlConfiguration
{
    public class RelationsSamples
    {
        ...
        public async Task ExplicitLoading()
        {
            ...
            {
                var movie = await context.Movies.FindAsync(1);
                await context.Entry(movie).Reference(e => e.Director).LoadAsync();
                await context.Entry(movie).Reference(e => e.Genre).LoadAsync();
            ...
            {
                var person = await context.People.FindAsync(1);
                await context.Entry(person).Collection(e => e.Movies).LoadAsync();
